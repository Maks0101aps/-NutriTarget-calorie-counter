...
exports.updateUserGoal = exports.createUserGoal = exports.getUserGoals = exports.updateDailyStats = exports.getWeeklyStats = exports.getDailyStats = void 0;
const zod_1 = require("zod");
const index_1 = require("../index");
// Validation schemas
const userGoalSchema = zod_1.z.object({
    startWeight: zod_1.z.number().positive(),
    targetWeight: zod_1.z.number().positive(),
    endDate: zod_1.z.string().refine((val) => !isNaN(Date.parse(val)), {
        message: 'Invalid date format',
    }),
});
const updateUserGoalSchema = zod_1.z.object({
    startWeight: zod_1.z.number().positive().optional(),
    targetWeight: zod_1.z.number().positive().optional(),
    endDate: zod_1.z.string().refine((val) => !isNaN(Date.parse(val)), {
        message: 'Invalid date format',
    }).optional(),
    achieved: zod_1.z.boolean().optional(),
});
// Get daily stats
const getDailyStats = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!dailyStats) {
            ...
            let fatGoal = 70;
            // Create new stats
            const newDailyStats = yield index_1.prisma.dailyStats.create({
                data: {
                    userId,
                    date: startDate,
                    calories: 0,
...
const getWeeklyStats = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const stats = yield index_1.prisma.dailyStats.findMany({
            where: {
                userId,
                date: {
                    gte: startDate,
...
// Update daily stats
const updateDailyStats = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
        ...
        const updatedStats = yield index_1.prisma.dailyStats.upsert({
            where: {
                userId_date: {
                    userId,
                    date: startDate,
...
const getUserGoals = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userId = req.user.id;
        // Get all user goals
        const goals = yield index_1.prisma.userGoal.findMany({
            where: { userId },
...
const createUserGoal = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
        ...
        yield index_1.prisma.userGoal.updateMany({
            where: {
                userId,
                achieved: false,
                endDate: {
        ...
        // Create new goal
        const newGoal = yield index_1.prisma.userGoal.create({
            data: {
                userId,
...
const updateUserGoal = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existingGoal = yield index_1.prisma.userGoal.findUnique({
            where: { id: goalId },
        });
        if (!existingGoal) {
        ...
        // Prepare update data
        const updateData = {};
        if (validatedData.startWeight !== undefined)
            updateData.startWeight = validatedData.startWeight;
        if (validatedData.targetWeight !== undefined)
        ...
        // Update goal
        const updatedGoal = yield index_1.prisma.userGoal.update({
            where: { id: goalId },
            data: updateData,
        });
